...
#10
select distinct user.nif, user.name, givesopinionconcert.points_rate_concert 
...
&& year(disc.editionDate) = "2018" && musicgroup.genre = "Metal" group by song.title;
#12
select distinct musician.name, disc.title,disc.song_format, disc.size_disc 
from musician, belongs, musicgroup, disc, creates, plays, instrument 
where disc.model_disc = "Digital" && musician.id = belongs.id_musician && belongs.id_group = musicgroup.id 
&& musicgroup.id = creates.id_musicGroup && creates.reference_id = disc.id  && musician.id = plays.id_musician 
&& instrument.name_instrument = plays.name_instrument && instrument.name_instrument = "Guitar" && musicgroup.genre = "Jazz";
#13
select distinct user.name, user.firstSurname, user.secondSurname 
...
WHERE musicgroup.id 
NOT IN (SELECT musicgroup.id 
FROM musicgroup, performs, concert 
WHERE musicgroup.id = performs.id_musicGroup && performs.code_concert = concert.code && '2017' = YEAR(concert.concertDate))
group by contains.title_song;